    ...
    def save_model(self, request, obj, form, change):        
        if change:
            ...
            if(not paid and paid != obj.paid):
                to_email = obj.ecsa_billing_email           
                subject = 'Confirmation of payment and member badge'
                message = render_to_string('accounts/emails/ecsa_payment_confirmation.html', { 'name': obj.name, 'year': year,'total_amount': total_amount})
                email = EmailMessage(subject, message, to=[to_email, obj.mainDelegate.email], )
                email.content_subtype = "html"
